...
if run_age is None or run_age > RUN_MAX:
    state, why = "stale", (
        "the refresh job has not run" if run_age is None
        else f"the refresh job last ran {run_age} days ago")
elif data_age > DATA_MAX:
    state, why = "stale", (
        f"the job is running (last run {run_age}d ago) but no new period has been "
        f"published in {data_age} days — check whether KTT vẫn gửi báo cáo tháng")
else:
